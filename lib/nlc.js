...
const classify = (classifierName, text) => {
	return listClassifiers()
		.then(classifiers => {
			const relevant = classifiers.filter(c => c.name.startsWith(classifierName));
...
// assume all other classifiers that start with <name> have a subset of training_data
const updateClassifier = (name, training_data) => {

	return listClassifiers()
		.then(classifiers => {

			const relevant = classifiers.filter(c => c.name.startsWith(name));

			if(relevant.length == 0)
				return createClassifier(name, training_data);

			return createClassifier(`${name}-${relevant.length}`, training_data)
		})
